...
flambe.display.Font.prototype = {
	...
	,layoutText: function(text,align,wrapWidth,letterSpacing,lineSpacing) {
		...
		return new flambe.display.TextLayout(this,text,align,wrapWidth,letterSpacing,lineSpacing);
	}
...
flambe.display.TextLayout = function(font,text,align,wrapWidth,letterSpacing,lineSpacing) {
	this.lines = 0;
	var _g = this;
	this._font = font;
	this._glyphs = [];
...
flambe.display.TextSprite = function(font,text) {
	...
	this._flags = flambe.util.BitSets.add(this._flags,64);
	var dirtyText = function(_,_1) {
		_g._flags = flambe.util.BitSets.add(_g._flags,64);
	};
	this.wrapWidth = new flambe.animation.AnimatedFloat(0,dirtyText);
	this.letterSpacing = new flambe.animation.AnimatedFloat(0,dirtyText);
	this.lineSpacing = new flambe.animation.AnimatedFloat(0,dirtyText);
};
...
flambe.display.TextSprite.prototype = $extend(flambe.display.Sprite.prototype,{
	...
	,updateLayout: function() {
		...
		if(flambe.util.BitSets.contains(this._flags,64)) {
			this._flags = flambe.util.BitSets.remove(this._flags,64);
	...
	,getNaturalHeight: function() {
		...
		return flambe.math.FMath.max(paddedHeight,boundsHeight);
	}
	,getNaturalWidth: function() {
		this.updateLayout();
...
var game = {}
game.Main = function() { }
game.Main.__name__ = true;
game.Main.main = function() {
	flambe.System.init();
	game.MainStage.init(960,560);
	var manifest = flambe.asset.Manifest.fromAssets("bootstrap");
	var loader = flambe.System.loadAssetPack(manifest);
	loader.get(game.Main.onSuccess);
}
game.Main.onSuccess = function(pack) {
	game.Main.setupBackgroundImage();
	game.Main.setupTextSprite(pack);
	game.Main.setupPlane(pack);
	game.Main.setupTouchListeners();
}
game.Main.setupBackgroundImage = function() {
	var backgroundSprite = new flambe.display.FillSprite(16777215,game.MainStage.get_width(),game.MainStage.get_height());
	flambe.System.root.addChild(new flambe.Entity().add(backgroundSprite));
}
game.Main.setupTextSprite = function(pack) {
	var stringToShow = "Flambe AutoStageResize Test\nby AccidentalRebel.\n\n" + "Resize your browser to see the stage resizing in action.\n\n" + "Click anywhere to change the position of the plane.";
	var arialFont = new flambe.display.Font(pack,"arial");
	var textSprite = new flambe.display.TextSprite(arialFont,stringToShow);
	textSprite.set_align(flambe.display.TextAlign.Center);
	textSprite.anchorY.set__(textSprite.getNaturalHeight() / 2);
	textSprite.setXY(game.MainStage.get_width() / 2,game.MainStage.get_height() / 2);
	flambe.System.root.addChild(new flambe.Entity().add(textSprite));
}
game.Main.setupPlane = function(pack) {
	game.Main._planeSprite = new flambe.display.ImageSprite(pack.getTexture("plane"));
	game.Main._planeSprite.centerAnchor();
	flambe.System.root.addChild(new flambe.Entity().add(game.Main._planeSprite));
}
game.Main.setupTouchListeners = function() {
	flambe.System.get_pointer().down.connect(game.Main.onTouchDown);
}
game.Main.onTouchDown = function(pointerEvent) {
	var adjustedPosition = game.MainStage.convertToMainStageCoordinates(new flambe.math.Point(pointerEvent.viewX,pointerEvent.viewY));
	game.Main._planeSprite.setXY(adjustedPosition.x,adjustedPosition.y);
}
game.MainStage = function() { }
game.MainStage.__name__ = true;
game.MainStage.init = function(designSizeWidth,designSizeHeight) {
	game.MainStage._designSizeWidth = designSizeWidth;
	game.MainStage._designSizeHeight = designSizeHeight;
	game.MainStage.setupMainStageSprite();
	game.MainStage.setupStageResizeListener();
	game.MainStage.resizeStage();
}
game.MainStage.convertToMainStageCoordinates = function(positionToConvert) {
	var xPos = (positionToConvert.x - game.MainStage._mainStageSprite.x.get__()) / game.MainStage._computedStageScale;
	var yPos = (positionToConvert.y - game.MainStage._mainStageSprite.y.get__()) / game.MainStage._computedStageScale;
	var convertedPosition = game.MainStage.capPositionToMainStage(new flambe.math.Point(xPos,yPos));
	return convertedPosition;
}
game.MainStage.setupMainStageSprite = function() {
	game.MainStage._mainStageSprite = new flambe.display.Sprite();
	game.MainStage._mainStageSprite.scissor = new flambe.math.Rectangle(0,0,game.MainStage._designSizeWidth,game.MainStage._designSizeHeight);
	flambe.System.root.add(game.MainStage._mainStageSprite);
}
game.MainStage.setupStageResizeListener = function() {
	flambe.System.get_stage().resize.connect(game.MainStage.onStageResize);
}
game.MainStage.onStageResize = function() {
	game.MainStage.resizeStage();
}
game.MainStage.resizeStage = function() {
	game.MainStage._computedStageScale = game.MainStage.computeScaleAccordingToNewStageDimensions();
	game.MainStage._mainStageSprite.setScale(game.MainStage._computedStageScale);
	game.MainStage.centerStage();
}
game.MainStage.computeScaleAccordingToNewStageDimensions = function() {
	var currentStageWidth = flambe.System.get_stage().get_width();
	var currentStageHeight = flambe.System.get_stage().get_height();
	var idealWidth = game.MainStage._designSizeWidth;
	var idealHeight = game.MainStage._designSizeHeight;
	var computedScale = 1;
...
game.MainStage.centerStage = function() {
	var xDiff = flambe.System.get_stage().get_width() - game.MainStage._designSizeWidth * game.MainStage._computedStageScale;
	var yDiff = flambe.System.get_stage().get_height() - game.MainStage._designSizeHeight * game.MainStage._computedStageScale;
	game.MainStage._mainStageSprite.x.set__(xDiff / 2);
	game.MainStage._mainStageSprite.y.set__(yDiff / 2);
}
game.MainStage.capPositionToMainStage = function(position) {
	if(position.x < 0) position.x = 0; else if(position.x > game.MainStage.get_width()) position.x = game.MainStage.get_width();
	if(position.y < 0) position.y = 0; else if(position.y > game.MainStage.get_height()) position.y = game.MainStage.get_height();
	return position;
}
game.MainStage.get_width = function() {
	return game.MainStage._designSizeWidth;
}
game.MainStage.get_height = function() {
	return game.MainStage._designSizeHeight;
}
var haxe = {}
haxe.Json = function() {
};
...
flambe.asset.Manifest.__meta__ = { obj : { assets : [{ bootstrap : [{ bytes : 62781, md5 : "a7f0486e907c26b10376c12b289b5705", name : "arial.fnt"},{ bytes : 73484, md5 : "82142e376cbc0361637b8fd3ca8af4af", name : "arial_0.png"},{ bytes : 3822, md5 : "8f6c1133edf354da44b4d1a3e3377c6c", name : "plane.png"}]}]}};
